-----------------------------------------------------------------------------
-- |
-- Module      :  Language.CoreErl.Parser
...
import Language.CoreErl.Syntax
...
patterns = liftM EPat pat <|>
           liftM EPats (angles $ commaSep pat)

pat :: Parser EPat
pat = liftM EPAlias (try alias) {- because of variable -} <|> liftM EPVar variable <|>
          liftM EPLit (try literal) {- because of nil -} <|> liftM EPTuple (tuple pat) <|>
          liftM EPList (elist pat) <|> liftM EPBinary (ebinary pat)
